...
char train_file[MAX_STRING_FILE],train_file_alter[MAX_STRING_FILE],train_file_alter2[MAX_STRING_FILE], output_file[MAX_STRING_FILE];
...
void TrainModel() {
  ...
  if (iter_saved == 1) {
    ...
    while(total_iter < actual_iter/2) {
      if (strlen(train_file_alter) > 4) {
        strcpy(train_file, train_file_alter2);
        for (a = 0; a < num_threads; a++) pthread_create(&pt[a], NULL, TrainModelThread, (void *)a);
        for (a = 0; a < num_threads; a++) pthread_join(pt[a], NULL);
        strcpy(train_file, train_file_alter);
      }
      for (a = 0; a < num_threads; a++) pthread_create(&pt[a], NULL, TrainModelThread, (void *)a);
      for (a = 0; a < num_threads; a++) pthread_join(pt[a], NULL);
      total_iter += iter;
  ...
  for (a = 0; a < num_threads; a++) pthread_join(pt[a], NULL);
  char buffer[MAX_STRING];
  sprintf(buffer, "_v%lld_n%d_i%i_dynamic_window.txt", layer1_size,negative,actual_iter);
  char output_filename[MAX_STRING_FILE];
  strcpy(output_filename,output_file);
  strcat(output_filename, buffer);
  fo = fopen(output_filename, "wb");
...
int main(int argc, char **argv) {
  ...
  if ((i = ArgPos((char *)"-train", argc, argv)) > 0) {
    ...
    strcpy(train_file_alter2, argv[i + 1]);
  }
  if ((i = ArgPos((char *)"-train-alternate", argc, argv)) > 0) strcpy(train_file_alter, argv[i + 1]);
